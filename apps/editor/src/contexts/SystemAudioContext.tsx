...
export function SystemAudioProvider({ children }: SystemAudioProviderProps) {
  ...
  // Convert Float32Array from VAD to Blob for transcription
  const convertVADAudioToBlob = useCallback((audioData: Float32Array): Blob => {
  ...
  const handleSystemSpeechEnd = useCallback(
    async (audioData: Float32Array) => {
      try {
        console.log(
          '🖥️ System VAD speech ended, audio length:',
          audioData.length
        )

        // Convert VAD audio to blob
        const audioBlob = convertVADAudioToBlob(audioData)
